      ...
      <div className="w-full max-w-[180px] flex flex-col gap-3">
        <div className="flex gap-2 items-center">
          ...
          <div className="h-2 bg-zinc-800 rounded-full w-full overflow-hidden relative">
            <div
              className={`absolute top-0 left-0 h-full bg-zinc-500 rounded-full ${
                inView ? 'animate-[expand-bar_2s_ease-out_forwards]' : 'opacity-0'
              }`}
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="h-2 bg-zinc-800 rounded-full w-full overflow-hidden relative">
            <div
              className={`absolute top-0 left-12 h-full bg-indigo-500 rounded-full ${
                inView ? 'animate-[expand-bar_2.5s_ease-out_forwards_0.5s]' : 'opacity-0'
              }`}
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="h-2 bg-zinc-800 rounded-full w-full overflow-hidden relative">
            <div
              className={`absolute top-0 right-0 h-full bg-zinc-600 rounded-full ${
                inView ? 'animate-[expand-bar_3s_ease-out_forwards_1s]' : 'opacity-0'
              }`}
  ...
  },
  {
    ...
    renderVisual: (inView) => (
      <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
        ...
        <div 
          className={`absolute w-10 h-10 bg-zinc-900 rounded-full border border-zinc-800 z-20 transition-all duration-1000 ease-out flex items-center justify-center ${
            inView ? 'opacity-100 scale-100' : 'opacity-40 scale-75'
          }`}
        >
           <div className={`w-3 h-3 bg-indigo-500 rounded-full transition-all duration-1000 ${inView ? 'shadow-[0_0_20px_rgba(99,102,241,0.8)] animate-pulse' : 'shadow-none opacity-50'}`} />
        </div>
        ...
        {[0, 120, 240].map((deg, i) => (
          <div
            key={i}
            className={`absolute flex items-center justify-center transition-opacity duration-1000 z-30 ${
          ...
          >
             ...
             >
               {/* The Idea Particle */}
               <div 
                 className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 rounded-full transition-all duration-1000 ${
                   i === 0 ? 'bg-indigo-500' : i === 1 ? 'bg-zinc-500' : 'bg-zinc-600'
                 }`}
                 style={{
                   boxShadow: inView 
                     ? i === 0 
        ...
        <div className={`absolute inset-0 bg-gradient-to-tr from-indigo-500/5 via-transparent to-zinc-500/5 rounded-full blur-2xl transition-opacity duration-1000 ${inView ? 'opacity-100' : 'opacity-0'}`} />
      </div>
    ),
  },
  {
    ...
    renderVisual: (inView) => (
      <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
        ...
        <div 
          className={`absolute transition-transform duration-[2.5s] ease-in-out z-10 ${
            inView ? 'translate-x-1 translate-y-[-4px] rotate-0' : 'translate-x-[-15px] translate-y-[8px] -rotate-12'
          }`}
        >
           <div className="relative w-[90px] h-[90px]">
             ...
             >
                ...
                <div 
                  className={`absolute inset-0 bg-[linear-gradient(rgba(99,102,241,0.2)_1px,transparent_1px),linear-gradient(90deg,rgba(99,102,241,0.2)_1px,transparent_1px)] bg-[size:10px_10px] transition-opacity duration-700 ${inView ? 'opacity-100' : 'opacity-0'}`} 
                />
                
                {/* The "Found" Insight Core */}
                <div className={`relative z-10 w-6 h-6 bg-indigo-500/20 rounded-full flex items-center justify-center backdrop-blur-sm transition-all duration-500 ${inView ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
                  <div className="w-2 h-2 bg-indigo-400 rounded-full animate-pulse shadow-[0_0_8px_rgba(99,102,241,1)]" />
                </div>
        ...
        {[0, 1, 2].map((i) => (
           <div
             key={i}
             className={`absolute w-1.5 h-1.5 bg-indigo-500 rounded-full transition-all duration-1000 ${inView ? 'opacity-100' : 'opacity-0'}`}
             style={{
               top: `${30 + i * 20}%`,
               right: `${15 + i * 5}%`,
    ...
    renderVisual: () => (
      <div className="relative w-full max-w-[220px] h-28 bg-zinc-900 border border-zinc-800 rounded-xl p-3 flex flex-col gap-2 overflow-hidden shadow-2xl">
        ...
        <div className="relative flex-1 w-full bg-zinc-950 rounded-lg border border-zinc-800 overflow-hidden">
          <div className="absolute inset-0 bg-indigo-600 flex items-center justify-between px-4">
    ...
    renderVisual: (inView) => (
      <div className="relative w-full h-full flex items-center justify-center p-4">
        <div className="grid grid-cols-2 gap-2 transform rotate-12 group-hover:rotate-0 transition-transform duration-700 ease-out">
          ...
          <div className={`w-12 h-12 bg-indigo-500 rounded-tr-xl z-0 ${inView ? 'animate-float' : ''}`} style={{ animationDelay: '2s' }} />
      ...
      <div className="w-full max-w-[200px] bg-zinc-950 border border-zinc-800 rounded p-3 font-mono text-[10px] leading-relaxed text-zinc-500 shadow-inner">
        ...
        <div className="space-y-1">
          ...
          <div className="pl-2 flex items-center">
            ...
            <span className={`inline-block w-16 h-2 bg-indigo-600 rounded ml-1 ${inView ? 'animate-pulse' : ''}`} />
